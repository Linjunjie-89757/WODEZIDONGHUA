...
const screenshotPath = `output/playwright/case-center-readonly-${stamp}.png`;
...
try {
  ...
  const directoriesLoaded = requests.some((request) =>
    request.url.includes('/api/cases/directories') &&
    request.method === 'GET' &&
    request.status === 200
  );
  const casesLoaded = requests.some((request) =>
  ...
  if (!directoriesLoaded || !casesLoaded) {
    throw new Error('Case center readonly API requests were not both observed.');
  }
