<template>
  <AppButton type="primary" @click="open">{{ t.aiConnection.create }}</AppButton>
  <AppDialog
    v-model:visible="visible"
    :title="t.aiConnection.createTitle"
  ...
  >
    <a-form :model="form" layout="vertical">
      ...
      <a-form-item field="apiKey" :label="t.aiConnection.fieldApiKey">
        <a-input-password
          v-model="form.apiKey"
          :placeholder="t.aiConnection.fieldApiKeyPlaceholder"
        />
      </a-form-item>
...
import { aiProtocolOptions } from '@entities/ai-model';
import { t } from '@shared/i18n';
import { AppButton, AppDialog } from '@shared/ui';

import { useAiConnectionCreate } from '../model/useAiConnectionCreate';

const emit = defineEmits<{
  success: [];
}>();

const { visible, saving, form, open, submit } = useAiConnectionCreate();
